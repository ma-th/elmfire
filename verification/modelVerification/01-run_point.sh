...
cp $ELMFIRE_BASE_DIR/build/source/building_fuel_models.csv $MISC  # MT - uncommented
...
for WIND_SPEED in 0.0 15.0 30.0; do
   ...
   echo "$OUTPUTS_DIR" # MT - lines added   
   ...
   #/home/dwip/binYiren/elmfire_$ELMFIRE_VER ./inputs/elmfire.data   # MT - commented out


   
   ELMFIRE_VER=${ELMFIRE_VER:-2025.0302}   # MT - lines added   
   export ELMFIRE_INSTALL_DIR="$ELMFIRE_BASE_DIR/build/linux/bin"   # MT - lines added
   ELMFIRE_INSTALL_DIR=${ELMFIRE_INSTALL_DIR:-$ELMFIRE_BASE_DIR/build/linux/bin}   # MT - lines added


   #ELMFIRE_VER=${ELMFIRE_VER:-2025.0916}   # MT - lines added   
   #export ELMFIRE_INSTALL_DIR="$ELMFIRE_BASE_DIR/buildYiren_SU/linux/bin"   # MT - lines added
   #ELMFIRE_INSTALL_DIR=${ELMFIRE_INSTALL_DIR:-$ELMFIRE_BASE_DIR/buildYiren_SU/linux/bin}   # MT - lines added


   ELMFIRE=$ELMFIRE_INSTALL_DIR/elmfire_$ELMFIRE_VER  # MT - lines added
   $ELMFIRE ./inputs/elmfire.data >& elmfire.out  # MT - lines added
   ...
   # Clean up and exit:
   rm -f -r $OUTPUTS_DIR/*.csv $OUTPUTS_DIR/*.bil $OUTPUTS_DIR/*.hdr ./inputs/ws.tif
...
for FUEL in $(seq 1 7); do
   ...
   # Clean up and exit:
   rm -f -r $OUTPUTS_DIR/*.csv $OUTPUTS_DIR/*.bil $OUTPUTS_DIR/*.hdr
