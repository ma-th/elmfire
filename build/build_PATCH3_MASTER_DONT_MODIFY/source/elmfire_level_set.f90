...
IF (.NOT. RANDOM_IGNITIONS) THEN
   ...
   DO IX = 1, NX
      IF (PHIP(IX,IY) .LE. 0.) THEN
         ...
         CALL APPEND_TO_DYNAMIC_ARRAY(IX, IY, LIST_BURNED%NUM_NODES, DYNAMIC_ARRAY)  ! DEBUG_YIREN
         ...
         DO K = 1, LIST_BURNED%NUM_NODES
            ...
            DO I = MAX(1,IX-1), MIN(NX,IX+1)
               IF ( (FBFM%I2(I,J,1) .EQ. 91) .AND. (POPULATION_DENSITY%R4(I,J,1) .GT. 0.) ) THEN
                  ...
                  CALL APPEND_TO_DYNAMIC_ARRAY(I, J, LIST_BURNED%NUM_NODES, DYNAMIC_ARRAY)  ! DEBUG_YIREN
...
! Dwi Speed Up Untag
!   IF ((.NOT. UNTAG_BECAUSE_BURNED) .AND. UNTAG_TYPE_4) THEN
 !     IF (C%IFBFM .EQ. 91 .AND. (TOA(IX,IY) .GT. 0. .AND. T - TOA(IX,IY) .GT. 7200.)) UNTAG_BECAUSE_BURNED = .TRUE. 
  ! ENDIF
! End Dwi SU
! Outputs # of cells in the LL runtime
