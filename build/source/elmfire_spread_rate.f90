...
INTEGER :: I, TEMP_SX, TEMP_SY, DEL_X, DEL_Y, IX_LOW_BORDER, IY_LOW_BORDER, IX_HIGH_BORDER, IY_HIGH_BORDER, &
           IX_TEMP, IY_TEMP, HAZ
REAL, INTENT(IN) :: T
REAL :: TARGET_R_METERS, TARGET_THETA, WIND_THETA, TARGET_THETA_F, MAX_ELLIPSE_DIST, ELLIPSE_DIST_THETA, &
        ...
        DFC_COEFF, RAD_COEFF, FTP_PA,ANALYSIS_CELLSIZE_SQUARED,RANALYSIS_CELLSIZE, HALF_ANALYSIS_CELLSIZE, &
        RDEL_X, RDEL_Y, ELLIPSE_MINOR_SQUARED, TOTAL_DFC, TOTAL_RADIATION, RAD_PER_SQCELL, &
        DFC_HEAT_FLUX, RAD_HEAT_FLUX, DFC_SIGMOID, RAD_SIGMOID, INCIDENT_HEAT_ADJUSTER, TARGET_WWSA, &  
        TARGET_BLDG_AREA_EFF, BLDG_AREA_DIM, BLDG_SSD, Q_CRIT
...
Q_CRIT = BUILDING_FUEL_MODEL_TABLE(C%IBLDGFM)%Q_CRIT

BLDG_AREA_DIM = BLDG_AREA%R4(C%IX,C%IY,1)
TARGET_WWSA = (2*BLDG_AREA_DIM*BUILDING_FUEL_MODEL_TABLE(C%IBLDGFM)%HEIGHT + &
               BLDG_AREA_DIM**2)*BLDG_FOOTPRINT_FRAC%R4(C%IX,C%IY,1) ! Target cell, bldg windward surface area + roof area
TARGET_BLDG_AREA_EFF = (BLDG_AREA_DIM**2)*BLDG_FOOTPRINT_FRAC%R4(C%IX,C%IY,1)
BLDG_SSD   = BLDG_SEPARATION_DIST%R4 (C%IX,C%IY,1)
...
DO I = 1, LB%NUM_NODES
   ...
   IF ((LB_P%IX .GE. IX_LOW_BORDER) .AND. (LB_P%IY .GE. IY_LOW_BORDER) .AND. (LB_P%IX .LE. IX_HIGH_BORDER) .AND. (LB_P%IY .LE. IY_HIGH_BORDER)) THEN
      ...
      INCIDENT_HEAT_ADJUSTER = (TARGET_WWSA)/(PI*LB_P%ELLIPSE_PARAMETERS%ELLIPSE_MAJOR*LB_P%ELLIPSE_PARAMETERS%ELLIPSE_MINOR)  !DWI_M2
      ...
      RDEL_X = REAL(DEL_X)*ANALYSIS_CELLSIZE
      RDEL_Y = REAL(DEL_Y)*ANALYSIS_CELLSIZE

      TARGET_R_METERS = SQRT( RDEL_X*RDEL_X + RDEL_Y*RDEL_Y)

      IF (TARGET_R_METERS .LT. 1E-6) THEN
         CYCLE
      ENDIF
   ...
   ! Direct Flame Contact
      ...
      DFC_HEAT_RECEIVED = DFC_COEFF*DFC_FACTOR*LB_P%HRR_TRANSIENT !kW/m2
   ...
   ! Radiation
      ...
      ELSE
         RAD_EFF_DIST = MAX(1E-6, TARGET_R_METERS - ELLIPSE_DIST_THETA)
      ENDIF

      RAD_HEAT_RECEIVED = INCIDENT_HEAT_ADJUSTER*(0.3*DFC_COEFF*RAD_COEFF*RAD_FACTOR*LB_P%HRR_TRANSIENT)/(4*PI*RAD_EFF_DIST*RAD_EFF_DIST) !kW/m2

      TOTAL_DFC = TOTAL_DFC + DFC_HEAT_RECEIVED*SIMULATION_DT !kJ/m2
      TOTAL_RADIATION = TOTAL_RADIATION + RAD_HEAT_RECEIVED*SIMULATION_DT !kJ/m2
   ...
   ! Assign ignition time
   IF ((C%TOTAL_DFC_RECEIVED + C%TOTAL_RAD_RECEIVED) .GT. Q_CRIT) THEN
      LB_P%T_BLDG_IGN = T
   ENDIF
...
C%ABSOLUTE_U = ((TOTAL_DFC + TOTAL_RADIATION)-Q_CRIT)*(BLDG_SSD)/(FTP_PA) !m/s
...
C%VELOCITY_DMS = 60*(C%ABSOLUTE_U * FLAME_FRONT / SUM_ELLIPSE) * 3.28084  ! ft/min
C%VBACK = 60*(C%ABSOLUTE_U * FLAME_BACK / SUM_ELLIPSE) * 3.28084    ! ft/min
V_S = 60*(C%ABSOLUTE_U * FLAME_SIDE / SUM_ELLIPSE) * 3.28084    ! ft/min


IF (V_S .GT. 1E-4) THEN
   C%LOW = AMIN1((C%VELOCITY_DMS+C%VBACK) / MAX(2 * V_S, 1E-6), 10.0)
ELSE
   C%LOW = 1.0
ENDIF
...
REAL ::  BURNING_TIME, T_1MW, EARLY_TIME, HRR_PEAK, DEVELOPED_TIME, DECAY_TIME, TOTAL_BURN_TIME
REAL, INTENT(IN) :: T

T_1MW = BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%T_1MW
...
TOTAL_BURN_TIME = T_1MW + EARLY_TIME + DEVELOPED_TIME + DECAY_TIME

HRR_PEAK = BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%HRRPUA_PEAK * &
         BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%NONBURNABLE_FRAC

IF (BURNING_NODES%IFBFM .NE. 91) HRR_PEAK = BURNING_NODES%FLIN_SURFACE / ANALYSIS_CELLSIZE

IF (BURNING_NODES%T_BLDG_IGN .GT. 0) THEN
   BURNING_TIME = T - BURNING_NODES%T_BLDG_IGN
ELSE
   ! If T_BLDG_IGN was not set, use TIME_OF_ARRIVAL as a fallback
   BURNING_TIME = T - BURNING_NODES%TIME_OF_ARRIVAL
ENDIF

BURNING_TIME = MAX(0.0, BURNING_TIME)

IF (BURNING_TIME .LE. T_1MW) THEN
   BURNING_NODES%HRR_TRANSIENT = (HRR_PEAK / EARLY_TIME) * (BURNING_TIME / T_1MW) ** 2
ELSEIF ((BURNING_TIME .GT. T_1MW) .AND. (BURNING_TIME .LE. EARLY_TIME)) THEN
   BURNING_NODES%HRR_TRANSIENT = (HRR_PEAK / (EARLY_TIME - T_1MW)) * (BURNING_TIME - T_1MW)
ELSEIF ((BURNING_TIME .GT. EARLY_TIME) .AND. (BURNING_TIME .LE. DEVELOPED_TIME)) THEN
   BURNING_NODES%HRR_TRANSIENT = HRR_PEAK
ELSEIF ((BURNING_TIME .GT. DEVELOPED_TIME) .AND. (BURNING_TIME .LE. DECAY_TIME)) THEN
   BURNING_NODES%HRR_TRANSIENT = (HRR_PEAK / (DEVELOPED_TIME - DECAY_TIME)) * (BURNING_TIME - DECAY_TIME)
ELSEIF (BURNING_TIME .GT. TOTAL_BURN_TIME) THEN
