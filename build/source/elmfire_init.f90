...
REAL :: T_1MW, T_EARLY, T_FULLDEV, T_DECAY, FUEL_LOAD, HRRPUA_PEAK, FTP_CRIT, Q_CRIT, &
      ABSORPTIVITY,HEIGHT, NONBURNABLE_FRAC, P_IGNITION, HARDENING_FACTCOR, TAU_IGN
INTEGER :: INUM, IOS

! Initialize building fuel model table:
BUILDING_FUEL_MODEL_TABLE(:)%SHORTNAME='NULL'
BUILDING_FUEL_MODEL_TABLE(:)%T_1MW=-9999.       ! Incipient stage or time to reach 1 MW, s
BUILDING_FUEL_MODEL_TABLE(:)%T_EARLY=-9999.     ! Early growth stage, s
BUILDING_FUEL_MODEL_TABLE(:)%T_FULLDEV=-9999.   ! Fully developed stage, s
BUILDING_FUEL_MODEL_TABLE(:)%T_DECAY=-9999.     ! Decay stage, s
BUILDING_FUEL_MODEL_TABLE(:)%FUEL_LOAD=-9999.   ! Building fuel load, kJ-s/m2 -> to be modified in units kJ/m2
BUILDING_FUEL_MODEL_TABLE(:)%HRRPUA_PEAK=-9999. ! kW/m2 -> to be modified as HRR_PEAK in units kW
BUILDING_FUEL_MODEL_TABLE(:)%FTP_CRIT=-9999.    ! Critical flux-time product, kJ/m2
BUILDING_FUEL_MODEL_TABLE(:)%Q_CRIT=-9999.      ! Critical heat flux for ignition, kW/m2
BUILDING_FUEL_MODEL_TABLE(:)%ABSORPTIVITY=-9999.! Radiative absorptivity 
BUILDING_FUEL_MODEL_TABLE(:)%HEIGHT=-9999.      ! Building height, m
BUILDING_FUEL_MODEL_TABLE(:)%NONBURNABLE_FRAC=-9999. !Building nonburnable fraction
BUILDING_FUEL_MODEL_TABLE(:)%P_IGNITION=-9999.  ! Probability of ember ignition
...
IOS = 0
DO WHILE (IOS .EQ. 0)
   READ(LUINPUT,*,IOSTAT=IOS) INUM,SHORTNAME,T_1MW,T_EARLY,T_FULLDEV,T_DECAY,FUEL_LOAD,HRRPUA_PEAK,FTP_CRIT,Q_CRIT,ABSORPTIVITY,HEIGHT,NONBURNABLE_FRAC,P_IGNITION, HARDENING_FACTCOR, TAU_IGN
