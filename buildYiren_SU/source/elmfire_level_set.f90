...
! Calculate x and y components of velocity from elliptical spread dimensions
      CALL UX_AND_UY_ELLIPTICAL(LIST_TAGGED, LIST_BURNED, SURFACE_ACCELERATION_FACTOR, ISTEP, T, DYNAMIC_ARRAY)  !DWI_SU HICCUP 2
...
! Note that per Thomas (1963) and Rothermel (1991), crown fire flame length is Lf=0.2*I^2/3
         ...
         !CALL SYSTEM('del DIAG_LSET.csv')
         !OPEN(199712051, FILE="DIAG_LSET.csv")
         !WRITE(199712051,*) DYNAMIC_ARRAY(1, :)

         TOTAL_ELEMENTS = SIZE(DYNAMIC_ARRAY, 1)

         !OPEN(199712053, FILE="DIAG_ELEM.csv")
         !WRITE(199712053,*) TOTAL_ELEMENTS

         !PRINT *, 'SLSET ELEMENTS:', DYNAMIC_ARRAY(LIST_BURNED%NUM_NODES, :)
...
INTEGER :: IASP, I, ILH, NITER, IT1, IT2
...
CALL ACCUMULATE_CPU_USAGE(65, IT1, IT2)
...
! Calculate length over width:
            ...
            CALL ACCUMULATE_CPU_USAGE(66, IT1, IT2)
            IF (USE_BLDG_SPREAD_MODEL .AND. C%IFBFM .EQ. 91) THEN  !DWI_SU HICCUP 3
               CONTINUE
               IF (BLDG_SPREAD_MODEL_TYPE .EQ. 1) CALL HAMADA(C, DYNAMIC_ARRAY) ! GET C%VELOCITY_DMS, C%VBACK & C%LOW
               IF (BLDG_SPREAD_MODEL_TYPE .EQ. 2) CALL UMD_UCB_BLDG_SPREAD(C, LB, T_ELMFIRE, DYNAMIC_ARRAY) ! GET C%VELOCITY_DMS, C%VBACK & C%LOW
               CONTINUE
            ENDIF
            CALL ACCUMULATE_CPU_USAGE(67, IT1, IT2)
