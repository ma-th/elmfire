...
! *****************************************************************************
SUBROUTINE HAMADA(C, DYNAMIC_ARRAY)
! *****************************************************************************
...
INTEGER :: IX_LOW_BORDER, IY_LOW_BORDER, IX_HIGH_BORDER, IY_HIGH_BORDER
...
! CALL BURNED_FILTER(DYNAMIC_ARRAY, ZERO_FILTERED, IX_LOW_BORDER, IY_LOW_BORDER, IX_HIGH_BORDER, IY_HIGH_BORDER)

!CALL SYSTEM('del DIAG_SRATE.csv')
!OPEN(199712052, FILE="DIAG_SRATE.csv")
!WRITE(199712052,*) DYNAMIC_ARRAY(2, :)

!PRINT *, 'TEST_ELEM:', SIZE(DYNAMIC_ARRAY), SIZE(ZERO_FILTERED)
...
!IF (INDEX < SIZE(LB%NODE_POINTERS)) THEN
!    IF (ASSOCIATED(LB%NODE_POINTERS(INDEX)%PTR)) PRINT *, "PTR is valid"
!    PTR => LB%NODE_POINTERS(INDEX)%PTR
!    PRINT *, "PTR now points to:", LOC(PTR)
!    PRINT *, PTR%IX, PTR%IY, PTR%ELLIPSE_PARAMETERS%DIST_DOWNWIND
!END IF

!DO I = 1, SIZE(INDEX_FILTERED)
!    PTR => LB%NODE_POINTERS(INDEX_FILTERED(I))%PTR
!    PRINT *, "Filtered Node IX:", PTR%IX, "IY:", PTR%IY, "ELL:", PTR%ELLIPSE_PARAMETERS%DIST_DOWNWIND
!END DO
...
DO I = 1, SIZE(INDEX_FILTERED)  !LB%NUM_NODES   !            

! CALL ACCUMULATE_CPU_USAGE(81, IT1, IT2)
...
CALL ACCUMULATE_CPU_USAGE(82, IT1, IT2)
...
CALL ACCUMULATE_CPU_USAGE(83, IT1, IT2)
...
CALL ACCUMULATE_CPU_USAGE(84, IT1, IT2)
...
CALL ACCUMULATE_CPU_USAGE(85, IT1, IT2)
...
CALL ACCUMULATE_CPU_USAGE(86, IT1, IT2)
...
IF ((TOTAL_DFC + TOTAL_RADIATION)>30000) THEN
   FTP_PA = 3000
ELSE
   FTP_PA = 3000000/AMAX1(1E-3, RAD_PER_SQCELL*RAD_PER_SQCELL)  !CHECK IT
ENDIF
...
CALL ACCUMULATE_CPU_USAGE(74, IT1, IT2)
...
CALL ACCUMULATE_CPU_USAGE(75, IT1, IT2)
...
IF (BURNING_NODES%IFBFM .NE. 91) THEN 
   HRR_PEAK = 250 !BURNING_NODES%FLIN_SURFACE / ANALYSIS_CELLSIZE
   !OPEN(20071205, FILE="vegHRR_DIAGNOSTIC.csv")
   !WRITE(20071205,*) HRR_PEAK
   !WRITE(*,*) 'UCB CALLED'
ENDIF
...
! *****************************************************************************
SUBROUTINE BURNED_FILTER_LONG(DYNAMIC_ARRAY, ZERO_FILTERED, IX_LOW_BORDER, IY_LOW_BORDER, IX_HIGH_BORDER, IY_HIGH_BORDER)
! *****************************************************************************


!DWI_SU variables
REAL, ALLOCATABLE, INTENT(INOUT), DIMENSION(:,:) :: DYNAMIC_ARRAY, ZERO_FILTERED  ! Dynamic array to store IX and IY
INTEGER :: TOTAL_ELEMENTS, IX_LOW_BORDER, IY_LOW_BORDER, IX_HIGH_BORDER, IY_HIGH_BORDER
REAL, ALLOCATABLE, DIMENSION(:) :: FILTERED_IX, FILTERED_IY, FILTER_COUNT
LOGICAL, ALLOCATABLE, DIMENSION(:) :: VALID_VALUES
INTEGER, ALLOCATABLE, DIMENSION(:) :: INDICES ! Array to store indices of valid values
...
! Initialize all VALID_VALUES to FALSE
VALID_VALUES = .FALSE.

! Apply the condition to check if the values lie within the defined borders
WHERE (DYNAMIC_ARRAY(:, 1) >= IX_LOW_BORDER .AND. DYNAMIC_ARRAY(:, 1) <= IX_HIGH_BORDER .AND. &
       DYNAMIC_ARRAY(:, 2) >= IY_LOW_BORDER .AND. DYNAMIC_ARRAY(:, 2) <= IY_HIGH_BORDER)
    VALID_VALUES = .TRUE.  ! These values are within the border range
END WHERE

! Allocate INDICES to store the indices of valid values
ALLOCATE(INDICES(TOTAL_ELEMENTS))

! Initialize INDICES to zero
INDICES = 0

! Use WHERE to store the indices of the valid values
WHERE (VALID_VALUES)
    INDICES = 1  ! This just marks where VALID_VALUES is TRUE
END WHERE

! Now, filter out the values using the INDICES
ALLOCATE(FILTERED_IX(SIZE(INDICES)))
ALLOCATE(FILTERED_IY(SIZE(INDICES)))

! Extract the valid indices into FILTERED_IX and FILTERED_IY
FILTERED_IX = DYNAMIC_ARRAY(INDICES, 1)  ! Filter out valid IX values
FILTERED_IY = DYNAMIC_ARRAY(INDICES, 2)  ! Filter out valid IY values

FILTER_COUNT = PACK(FILTERED_IX, VALID_VALUES)
ALLOCATE(ZERO_FILTERED(SIZE(FILTER_COUNT), 2))

ZERO_FILTERED(:,1) = PACK(FILTERED_IX, VALID_VALUES)
ZERO_FILTERED(:,2) = PACK(FILTERED_IY, VALID_VALUES)

! *****************************************************************************
END SUBROUTINE BURNED_FILTER_LONG
! *****************************************************************************
